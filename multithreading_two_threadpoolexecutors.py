...
def handle_response(rqueue, vqueue, notfounds):
    ...
    while True:
        # break out of loop if ^C signal received or if lookups are done and queue is empty
        if stop_event.is_set() or (lookups_done.is_set() and not rqueue):
            break
...
if __name__ == "__main__":
    ...
    while not stop_event.is_set():
        ...
        with confute.ThreadPoolExecutor(max_workers=12) as call_pool, confute.ThreadPoolExecutor(max_workers=2) as route_pool:
            logger.info(f'submitting {len(docids_synthetic)} docids for validation by target func "{request_data.__name__}" to the pool')
            call_futures = [call_pool.submit(request_data, docid, request_url, response_queue) for docid in docids_synthetic]
            route_futures = [route_pool.submit(handle_response, response_queue, validated_queue, notfounds) for _ in range(2)]
            # wait for all the validation tasks to be done
            confute.wait(call_futures, return_when=confute.ALL_COMPLETED)
        ...
        # exit out of validation loop in main thread - either when threadpools are finished or if ^C received
        time.sleep(1)
        stop_event.set()
        logger.info('shutting down threadpools...')
        time.sleep(1)
        logger.info('exiting validation loop')
